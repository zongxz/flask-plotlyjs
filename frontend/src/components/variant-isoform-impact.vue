<template>
  <div>
    <el-card
      class="box-card"
      style="margin-top: 10px;">
      <template #header>
        <div class="card-header">
          <span class="section-title">Variant Isoform Impact</span>
        </div>
      </template>
      <el-radio-group v-model="radioButton1" size="medium">
        <el-radio-button :label="true">1</el-radio-button>
      </el-radio-group>
      <el-radio-group v-model="radioButton1" size="medium">
        <el-radio-button :label="false">2</el-radio-button>
      </el-radio-group>
<!--      <div-->
<!--        v-loading="loading"-->
<!--        element-loading-background="rgba(255, 255, 255, 1)"-->
<!--        style="z-index: 0"-->
<!--        v-show="radioButton1">-->
<!--        <div-->
<!--          v-if="isPlot">-->
<!--          <Plot-->
<!--            :data="data"-->
<!--            :layout="layout"-->
<!--          />-->
<!--        </div>-->
<!--        <div v-else>-->
<!--          <h3>There is no corresponding data published yet, we will update it when such data available.</h3>-->
<!--        </div>-->
<!--      </div>-->
      <div
      v-show="radioButton1">
        <AllDnmsOnAllTranscript/>
      </div>
      <div
      v-show="!radioButton1">
        <OneDnmsOnAllTranscript/>
      </div>
    </el-card>
    <el-card class="box-card" style="margin-top: 10px;">
      <template #header>
        <div class="card-header">
          <span class="section-title">Transcripts expression in tissues (data source: GTEx Analysis Release V8)</span>
        </div>
      </template>
      <div>
        <GtexTranscriptsExpression/>
      </div>
    </el-card>
    <el-card class="box-card" style="margin-top: 10px;">
      <template #header>
        <div class="card-header">
          <span class="section-title">Protein isoforms expression in tissues (data source: eGTEx Analysis)</span>
        </div>
      </template>
      <div>
        <ProteinIsoformExpress/>
      </div>
    </el-card>
  </div>
</template>
<script>
// import Plot from 'vue-plotly.js'
import { getVariantIsoforms } from '@/api/variant'
import GtexTranscriptsExpression from '@/views/variant/GtexTranscriptsExpression'
import AllDnmsOnAllTranscript from '@/views/variant/AllDnmsOnAllTranscript'
import OneDnmsOnAllTranscript from '@/views/variant/OneDnmsOnAllTranscript'
// import ProteinIsoformExpress from '@/views/variant/proteomicsdb_protein_expression'
import ProteinIsoformExpress from '@/views/variant/ProteinIsoformExpress'
// import eGtexIsoformExpression from '@/views/variant/eGtexIsoformExpression'
// import VariantProteinImpact from '@/views/variant/variant-protein-impact'
export default {
  name: 'VariantIsoformImpact',
  components: {
    GtexTranscriptsExpression, ProteinIsoformExpress, AllDnmsOnAllTranscript, OneDnmsOnAllTranscript
  },
  props: {
    variant_info: {}
  },
  data() {
    return {
      data: [],
      layout: {},
      isPlot: true,
      loading: true,
      radioButton1: true
    }
  },
  methods: {
    isformImpactPlot(params) {
      getVariantIsoforms(params).then(res => {
        function get_list_2D_region(list_2D) {
          const start_list = []
          const end_list = []
          for (const item of list_2D) {
            start_list.push(item[0])
            end_list.push(item[1])
          }
          const min_value = Math.min(Math.min(start_list), Math.min(end_list))
          const max_value = Math.max(Math.max(start_list), Math.max(end_list))
          return [min_value, max_value]
        }

        function get_list_2D_start_end(list_2D) {
          if (list_2D !== undefined) {
            const list_2D_flatten = list_2D[0]
            const sorted_list = list_2D_flatten.sort()
            return [sorted_list[0], sorted_list[sorted_list.length - 1]]
          }
          return [0, 0]
        }

        function get_transcript_region(strand, trans_structure) {
          if (strand !== undefined) {
            if (strand === '+') {
              if (trans_structure['utr5_exon_region'] !== undefined) {
                if (trans_structure['utr3_exon_region']) {
                  return [get_list_2D_start_end(trans_structure['utr5_exon_region'])[1],
                    get_list_2D_start_end(trans_structure['utr3_exon_region'])[0]]
                } else {
                  if (trans_structure['coding_exon_region']) {
                    return [get_list_2D_start_end(trans_structure['utr5_exon_region'])[1],
                      get_list_2D_region(trans_structure['coding_exon_region'])[1]]
                  } else {
                    return []
                  }
                }
              } else {
                if (trans_structure['coding_exon_region']) {
                  if (trans_structure['utr3_exon_region']) {
                    return [get_list_2D_start_end(trans_structure['coding_exon_region'])[0],
                      get_list_2D_start_end(trans_structure['utr3_exon_region'])[0]]
                  } else {
                    return get_list_2D_region(trans_structure['coding_exon_region'])
                  }
                } else {
                  return []
                }
              }
            } else {
              if (trans_structure['utr3_exon_region'] !== undefined) {
                if (trans_structure['utr5_exon_region']) {
                  return [get_list_2D_start_end(trans_structure['utr3_exon_region'])[1],
                    get_list_2D_start_end(trans_structure['utr5_exon_region'])[0]]
                } else {
                  if (trans_structure['coding_exon_region']) {
                    return [get_list_2D_start_end(trans_structure['utr5_exon_region'])[1],
                      get_list_2D_region(trans_structure['coding_exon_region'])[1]]
                  } else {
                    return []
                  }
                }
              } else {
                if (trans_structure['coding_exon_region']) {
                  if (trans_structure['utr3_exon_region']) {
                    return [get_list_2D_start_end(trans_structure['coding_exon_region'])[0],
                      get_list_2D_start_end(trans_structure['utr3_exon_region'])[0]]
                  } else {
                    return get_list_2D_region(trans_structure['coding_exon_region'])
                  }
                } else {
                  return []
                }
              }
            }
          } else {
            return []
          }
        }

        function get_trans_data(one_trans_structure, transcript_id, strand) {
          const res = []
          res.push(transcript_id)
          res.push(get_transcript_region(strand, one_trans_structure))
          if (one_trans_structure['utr3_exon_region'] !== undefined) {
            res.push(get_list_2D_start_end(one_trans_structure['utr3_exon_region']))
          } else {
            res.push([])
          }
          if (one_trans_structure['utr5_exon_region'] !== undefined) {
            res.push(get_list_2D_start_end(one_trans_structure['utr5_exon_region']))
          } else {
            res.push([])
          }
          return res
        }

        function drop_duplication(a_list, b_list, c_list) {
          const a_copy_list = []
          const b_copy_list = []
          const c_copy_list = []
          for (let i = 0; i < c_list.length; i++) {
            const item = c_list[i]
            if (!(item in c_copy_list)) {
              c_copy_list.push(item)
              b_copy_list.push(b_list[i])
              a_copy_list.push(a_list[i])
            }
          }
          return [a_copy_list, b_copy_list, c_copy_list]
        }

        function get_promoter_and_enhancer_data(promoter_data, enhancer_data, dnm_data) {
          const promoter_res = []
          const enhancer_res = []
          const temp_promoter = []
          const temp_promoter_frag = []
          const temp_promoter_tips = []
          if (promoter_data !== undefined) {
            for (const item_pro of promoter_data) {
              if (item_pro['start'] && item_pro['end']) {
                const tips = item_pro['short_descripton'] + ', ' + item_pro['cage_peak_id']
                temp_promoter.push('Promoter')
                temp_promoter_frag.push([item_pro['start'], item_pro['end']])
                temp_promoter_tips.push(tips)
              }
            }
          }
          const temp_enhancer = []
          const temp_enhancer_frag = []
          const temp_enhancer_tips = []
          if (enhancer_data !== undefined) {
            for (const item_enh of enhancer_data) {
              if (item_enh['start'] && item_enh['end']) {
                const tips = item_enh['genehancer_id'] + ', [' + item_enh['start'] + ' - ' +
                  item_enh['end'] + '], local'
                temp_enhancer.push('Enhancer')
                temp_enhancer_frag.push([item_enh['start'], item_enh['end']])
                temp_enhancer_tips.push(tips)
              }
            }
          }
          if (dnm_data !== undefined) {
            for (const dnm_item of dnm_data) {
              if (dnm_item['annotation'] !== undefined) {
                if (dnm_item['annotation']['enhancer_id'] !== undefined) {
                  if (dnm_item['re-annotation']['enh_start'] && dnm_item['re-annotation']['enh_end']) {
                    const tips = dnm_item['re-annotation']['enhancer_id'] + ', [' +
                      dnm_item['re-annotation']['enh_start'].replace('"', '') + '-' +
                      dnm_item['re-annotation']['enh_end'].replace('"', '') + '], targetgene'
                    temp_enhancer.push('Enhancer')
                    temp_enhancer_frag.push([Number(dnm_item['re-annotation']['enh_start']),
                      Number(dnm_item['re-annotation']['enh_end'])])
                    temp_enhancer_tips.push(tips)
                  }
                }
                if (dnm_item['re-annotation']['promoter_id'] !== undefined) {
                  if (dnm_item['re-annotation']['pro_start'] && dnm_item['re-annotation']['pro_end']) {
                    const tips = dnm_item['re-annotation']['promoter_id'] + ', [' +
                      dnm_item['re-annotation']['pro_start'].replace('"', '') + '-' +
                      dnm_item['re-annotation']['pro_end'].replace('"', '') + '], targetgene'
                    temp_promoter.push('Promoter')
                    temp_promoter_frag.push([Number(dnm_item['re-annotation']['pro_start']),
                      Number(dnm_item['re-annotation']['pro_end'])])
                    temp_promoter_tips.push(tips)
                  }
                }
              }
            }
          }
          if (temp_enhancer !== undefined && temp_enhancer_frag !== undefined && temp_enhancer_tips !== undefined) {
            const temp_enhancer_new = drop_duplication(temp_enhancer, temp_enhancer_frag, temp_enhancer_tips)[0]
            const temp_enhancer_frag_new = drop_duplication(temp_enhancer, temp_enhancer_frag, temp_enhancer_tips)[1]
            const temp_enhancer_tips_new = drop_duplication(temp_enhancer, temp_enhancer_frag, temp_enhancer_tips)[2]
            enhancer_res.push(temp_enhancer_new)
            enhancer_res.push(temp_enhancer_frag_new)
            enhancer_res.push(temp_enhancer_tips_new)
          }
          if (temp_promoter !== undefined && temp_promoter_frag !== undefined && temp_promoter_tips !== undefined) {
            const temp_promoter_new = drop_duplication(temp_promoter, temp_promoter_frag, temp_promoter_tips)[0]
            const temp_promoter_frag_new = drop_duplication(temp_promoter, temp_promoter_frag, temp_promoter_tips)[1]
            const temp_promoter_tips_new = drop_duplication(temp_promoter, temp_promoter_frag, temp_promoter_tips)[2]
            promoter_res.push(temp_promoter_new)
            promoter_res.push(temp_promoter_frag_new)
            promoter_res.push(temp_promoter_tips_new)
          }
          return [promoter_res, enhancer_res]
        }

        function get_dnms_data(dnms_data) {
          const dnm_list = []
          if (dnms_data !== undefined) {
            for (const dnm_item of dnms_data) {
              if (dnm_item['variant'] === undefined) {
                dnm_item['variant'] = dnm_item['start']
              }
              // dnm_item['variant'] = dnm_item['variant_description']
              let s = ''
              if (dnm_item['Func_refGene'] === 'exonic') {
                if (dnm_item['ExonicFunc_refGene'] !== undefined) {
                  s = dnm_item['variant'] +
                    ',' + dnm_item['ExonicFunc_refGene']
                } else {
                  s = dnm_item['variant']
                }
              } else {
                if (dnm_item['Func_refGene'] !== undefined) {
                  s = dnm_item['variant'] +
                    ',' + dnm_item['Func_refGene']
                } else {
                  s = dnm_item['variant']
                }
              }
              dnm_list.push([dnm_item['start'], 'Variant', s])
            }
          } else {
            dnm_list.push([])
          }
          return dnm_list
        }

        function hb_list_2D(test) {
          let i = 0
          let l = test.length
          while (i < l - 1) {
            if (test[i][1] === test[i + 1][0]) {
              test[i] = [test[i][0], test[i + 1][1]]
              test.splice(i + 1, 1)
              l = l - 1
            } else {
              i += 1
            }
          }
          return test
        }

        function hb_list_3D(list1) {
          for (let i = 0; i < list1.length; i++) {
            if (list1[i].length !== 0) {
              list1[i] = hb_list_2D(list1[i])
            }
          }
          return list1
        }

        function get_all_region(trans_data, regulatory_element_data) {
          const all_region = []
          for (const item of trans_data) {
            if (item[1].length !== 0) {
              all_region.push(item[1])
            }
            if (item[2].length !== 0) {
              all_region.push(item[2])
            }
            if (item[3].length !== 0) {
              all_region.push(item[3])
            }
          }
          for (const item of regulatory_element_data) {
            if (item.length !== 0) {
              for (const item_item of item[1]) {
                if (item_item.length !== 0) {
                  all_region.push(item_item)
                }
              }
            }
          }
          return all_region
        }

        // 判断二维数组array中是否存在一维数组element
        function judgment_array(array, element) {
          for (const el of array) {
            if (el.length === element.length) {
              for (const index in el) {
                if (el[index] !== element[index]) {
                  break
                }
                if (Number(index) === (el.length - 1)) {
                  return true
                }
              }
            }
          }
          return false
        }

        function drop_duplicate(one_list) {
          const temp_list = []
          for (const one of one_list) {
            if (!judgment_array(temp_list, one)) {
              temp_list.push(one)
            }
          }
          return temp_list
        }

        function sort_list(all_region) {
          const l = all_region.length
          all_region.sort()
          for (let i = 0; i < l - 1; i++) {
            console.assert(all_region[i][0] <= all_region[i + 1][0], '')
          }
          return all_region
        }

        function cut_fragment(all_region) {
          const all_s = []
          const all_e = []
          for (const item of all_region) {
            all_s.push(item[0])
            all_e.push(item[1])
          }
          let all_se = []
          for (const item of all_s) {
            all_se.push(item)
          }
          for (const item of all_e) {
            all_se.push(item)
          }
          const new_all_se = new Set(all_se.sort())
          all_se = Array.from(new_all_se)
          const a = all_se.slice(0, all_se.length - 1)
          const b = all_se.slice(1)
          const all_frag = []
          for (let i = 0; i < a.length; i++) {
            all_frag.push([a[i], b[i]])
          }
          return all_frag
        }

        function get_every_frag_dnm_numbers(all_frag, DNM) {
          let DNM_list = []
          for (const item of DNM) {
            DNM_list.push(item[0])
          }
          const new_DNM_list = new Set(DNM_list)
          DNM_list = Array.from(new_DNM_list)
          const num = Array(all_frag.length).fill(0)
          let i = 0
          for (const item of all_frag) {
            // eslint-disable-next-line no-unused-vars
            let k = 0
            for (const item_item of DNM_list) {
              if (item_item >= item[0] && item_item <= item[1]) {
                num[i] += 1
              }
              k += 1
            }
            i += 1
          }
          return num
        }

        function get_sum(x_list, n) {
          let sum = 0
          for (let i = 0; i < n; i++) {
            sum += x_list[i]
          }
          return sum
        }

        function scale_fragment(all_frag, DNM, regulatory_element_data) {
          const ll = []
          const pe = []
          const num = get_every_frag_dnm_numbers(all_frag, DNM)
          if (regulatory_element_data[0].length !== 0) {
            const promoter_frag = regulatory_element_data[0][1]
            for (const item of promoter_frag) {
              pe.push(item)
            }
          }
          if (regulatory_element_data[1].length !== 0) {
            const enhancer_frag = regulatory_element_data[1][1]
            for (const item of enhancer_frag) {
              pe.push(item)
            }
          }
          const pe_drop_dupliction = []
          for (const item of pe) {
            if (!(item in pe_drop_dupliction)) {
              pe_drop_dupliction.push(item)
            }
          }
          const flag = Array(all_frag.length).fill(0)
          for (let i = 0; i < all_frag.length; i++) {
            const item = all_frag[i]
            for (const item1 of pe_drop_dupliction) {
              if (item[0] >= item1[0] && item[1] <= item1[1]) {
                flag[i] = 1
              }
            }
          }
          for (let i = 0; i < flag.length; i++) {
            const item1 = flag[i]
            const item2 = num[i]
            if (item1 === 1) {
              ll.push(10)
            } else {
              if (item2 < 3) {
                ll.push(22)
              } else {
                ll.push(item2 * 10 + 2)
              }
            }
          }
          const re_a = []
          for (let i = 0; i < ll.length; i++) {
            re_a.push(get_sum(ll, i))
          }
          const re_b = []
          for (let i = 1; i < ll.length + 1; i++) {
            re_b.push(get_sum(ll, i))
          }
          const scale_frag = []
          for (let i = 0; i < re_a.length; i++) {
            scale_frag.push([re_a[i], re_b[i]])
          }
          return scale_frag
        }

        function count_exton_region_in_fragment(all_frag, scale_frag, trans_data) {
          const ae = []
          for (const item of trans_data) {
            ae.push(item[1])
          }
          let count = []
          let count1 = []
          for (let i = 0; i < ae.length; i++) {
            const item = ae[i]
            const temp1 = []
            const temp2 = []
            if (item.length === 0) {
              temp1.push([])
              temp2.push([])
              count.push(temp1)
              count1.push(temp2)
            } else {
              for (let j = 0; j < all_frag.length; j++) {
                const item2 = all_frag[j]
                if (item2[0] >= item[0] && item2[1] <= item[1]) {
                  temp1.push(scale_frag[j])
                  temp2.push(all_frag[j])
                }
              }
              count.push(temp1)
              count1.push(temp2)
            }
          }
          const sort_count = count
          for (let i = 0; i < sort_count.length; i++) {
            const item = sort_count[i]
            if (item !== [[]] && item !== []) {
              sort_count[i].sort(function(x, y) {
                return x[0] - y[0]
              })
            }
          }
          count = hb_list_3D(count)
          count1 = hb_list_3D(count1)
          try {
            for (let i = 0; i < count1.length; i++) {
              const item = count1[i]
              if (item !== [[]] && item !== []) {
                count1[i].sort(function(x, y) {
                  return x[0] - y[0]
                })
              }
            }
            // eslint-disable-next-line no-empty
          } catch (e) {
          }
          return [count, count1]
        }

        function count_utr_region_in_fragment(all_frag, scale_frag, trans_data) {
          const ae_utr = []
          for (const item of trans_data) {
            const temp = []
            if (item[2].length !== 0) {
              temp.push(item[2])
            }
            if (item[3].length !== 0) {
              temp.push(item[3])
            }
            ae_utr.push(temp)
          }
          let count = []
          let count1 = []
          for (const item of ae_utr) {
            const temp1 = []
            const temp2 = []
            if (item === [[]] || item === []) {
              temp1.push([])
              temp2.push([])
              count.push(temp1)
              count1.push(temp2)
            } else {
              for (let i = 0; i < item.length; i++) {
                const item1 = item[i]
                for (let j = 0; j < all_frag.length; j++) {
                  const item2 = all_frag[j]
                  if (item2[0] >= item1[0] && item2[1] <= item1[1]) {
                    temp1.push(scale_frag[j])
                    temp2.push(all_frag[j])
                  }
                }
              }
              count.push(temp1)
              count1.push(temp2)
            }
          }
          const sort_count = count
          for (let i = 0; i < sort_count.length; i++) {
            const item = sort_count[i]
            if (item !== [[]] && item !== []) {
              sort_count[i].sort(function(x, y) {
                return x[0] - y[0]
              })
            }
          }
          count = hb_list_3D(count)
          count1 = hb_list_3D(count1)
          for (let i = 0; i < count1.length; i++) {
            const item = count1[i]
            if (item !== [[]] && item !== []) {
              count1[i].sort(function(x, y) {
                return x[0] - y[0]
              })
            }
          }
          return [count, count1]
        }

        function count_promoter_region_in_fragment(all_frag, scale_frag, regulatory_element_data) {
          if (regulatory_element_data[0].length !== 0) {
            const ae_promoter = regulatory_element_data[0][1]
            let count = []
            let count1 = []
            for (let i = 0; i < ae_promoter.length; i++) {
              const item = ae_promoter[i]
              const temp = []
              const temp1 = []
              for (let j = 0; j < all_frag.length; j++) {
                const item2 = all_frag[j]
                if (item2[0] >= item[0] && item2[1] <= item[1]) {
                  temp.push(scale_frag[j])
                  temp1.push(all_frag[j])
                }
              }
              count.push(temp)
              count1.push(temp1)
            }
            count = hb_list_3D(count)
            count1 = hb_list_3D(count1)
            return [count, count1]
          }
        }

        function count_enhancer_region_in_fragment(all_frag, scale_frag, regulatory_element_data) {
          if (regulatory_element_data[1].length !== 0) {
            const ae_enhancer = regulatory_element_data[1][1]
            let count = []
            let count1 = []
            for (let i = 0; i < ae_enhancer.length; i++) {
              const item = ae_enhancer[i]
              const temp = []
              const temp1 = []
              for (let j = 0; j < all_frag.length; j++) {
                const item2 = all_frag[j]
                if (item2[0] >= item[0] && item2[1] <= item[1]) {
                  temp.push(scale_frag[j])
                  temp1.push(all_frag[j])
                }
              }
              count.push(temp)
              count1.push(temp1)
            }
            count = hb_list_3D(count)
            count1 = hb_list_3D(count1)
            return [count, count1]
          }
        }

        function convert_trans_to_xy(count, count1, trans_data) {
          const y = []
          for (const item of trans_data) {
            if (!(item[1].length === 0 && item[2].length === 0 && item[3].length === 0)) {
              y.push(item[0])
            }
          }
          const xy = []
          for (let i = 0; i < y.length; i++) {
            const item = count[i]
            const yy = y[i]
            const z = count1[i]
            for (let j = 0; j < item.length; j++) {
              const item_item = item[j]
              const item_tips = z[j]
              const temp = [[item_item[0], yy, item_tips[0]], [item_item[1], yy, item_tips[1]]]
              xy.push(temp)
            }
          }
          return xy
        }

        function convert_utr_to_xy(count, count1, trans_data) {
          const y = []
          for (const item of trans_data) {
            if (!(item[1].length === 0 && item[2].length === 0 && item[3].length === 0)) {
              y.push(item[0])
            }
          }
          const xy = []
          for (let i = 0; i < y.length; i++) {
            const item = count[i]
            const yy = y[i]
            const z = count1[i]
            for (let j = 0; j < item.length; j++) {
              const item_item = item[j]
              const item_tips = z[j]
              const temp = [[item_item[0], yy, item_tips[0]], [item_item[1], yy, item_tips[1]]]
              xy.push(temp)
            }
          }
          return xy
        }

        function convert_regulatory_elements_to_xy(count, p_or_e, regulatory_element_data) {
          const xy = []
          let tips = ''
          if (p_or_e === 'Promoter') {
            tips = regulatory_element_data[0][2]
          } else if (p_or_e === 'Enhancer') {
            tips = regulatory_element_data[1][2]
          } else {
            tips = 'Error'
          }
          for (let i = 0; i < count.length; i++) {
            const item = count[i]
            let temp = []
            for (const item_item of item) {
              temp = [[item_item[0], p_or_e, tips[i]], [item_item[1], p_or_e, tips[i]]]
              xy.push(temp)
            }
          }
          return xy
        }

        function DNM_sort_by_site(DNM) {
          if (DNM.length !== 0) {
            DNM.sort()
            return DNM
          } else {
            return []
          }
        }

        function mapping_mutation_to_frag(all_frag, scale_frag, DNM = []) {
          const DNM_list = []
          for (const item of DNM) {
            DNM_list.push(item[0])
          }
          const DNM_tips = []
          for (const item of DNM) {
            DNM_tips.push(item[1])
          }
          const un_sorted_DNM_hover = []
          for (const item of DNM) {
            un_sorted_DNM_hover.push(item[2])
          }
          const DNM_hover = un_sorted_DNM_hover.sort()
          const num = Array(all_frag.length).fill(0)
          let i = 0
          const DNM_frag = []
          for (const item of all_frag) {
            const temp = []
            let kk = 0
            for (const item_item of DNM_list) {
              if (item_item >= item[0] && item_item <= item[1]) {
                num[i] += 1
                const temp1 = []
                temp1.push(item_item)
                temp1.push(DNM_tips[kk])
                temp1.push(DNM_hover[kk])
                temp.push(temp1)
              }
              kk += 1
            }
            DNM_frag.push(temp)
            i += 1
          }
          DNM = []
          let j = 0
          for (const item of num) {
            if (item !== 0) {
              let start = scale_frag[j][0] + 2
              const temp = []
              for (let k = 0; k < item; k++) {
                const temp1 = []
                temp1.push(start)
                temp1.push(start + 3)
                temp1.push(DNM_frag[j][k][0])
                temp1.push(DNM_frag[j][k][1])
                temp1.push(DNM_frag[j][k][2])
                temp.push(temp1)
                start += 10
              }
              DNM.push(temp)
            }
            j += 1
          }
          const final_DNM = []
          for (const item of DNM) {
            for (const item_item of item) {
              final_DNM.push(item_item)
            }
          }
          return final_DNM
        }

        function convert_dnms_to_xy(dnm_data) {
          const xy = []
          for (const item of dnm_data) {
            const temp = [[item[0], item[3], item[4]], [item[1], item[3], item[4]]]
            xy.push(temp)
          }
          return xy
        }

        function get_promoter_color(text) {
          var s = text.split(',')
          var len = s.length
          s = s[len - 1]
          var color
          if (s === 'permissive') {
            color = 'rgb(234,255,0)'
          } else {
            color = 'rgb(255,165,0)'
          }
          return color
        }

        function get_enhancer_color(text) {
          var s = text.split(',')
          var len = s.length
          s = s[len - 1].trim()
          var color
          if (s === 'targetgene') {
            color = 'rgb(0,100,0)'
          } else {
            color = 'rgb(144,238,144)'
          }
          return color
        }

        function get_DNM_color(m_type) {
          // var color
          // if ((['frameshift deletion', 'frameshift insertion', 'frameshift substitution',
          //   'splice-site mutation', 'stopgain'].includes(m_type)) || m_type.startsWith('frameshift')) {
          //   color = 'red'
          // } else if ((['nonframeshift deletion', 'nonframeshift insertion', 'nonsynonymous snv',
          //   'nonframeshift substitution', 'nonsynonymous SNV', 'strat gain', 'start gain',
          //   'stoploss'].includes(m_type)) || m_type.startsWith('nonframeshift')) {
          //   color = 'pink'
          // } else if (['synonymous SNV', 'exonic', 'coding complex', 'synonymous snv'].includes(m_type)) {
          //   color = 'purple'
          // } else if (['unknown'].includes(m_type)) {
          //   color = 'yellow'
          // } else {
          //   color = 'blue'
          // }
          return 'red'
        }

        function get_heights(length) {
          if (length <= 2) {
            return 200
          } else if (length <= 4) {
            return 400
          } else if (length <= 10) {
            return 600
          } else if (length <= 13) {
            return 800
          } else {
            return length * 30
          }
        }

        const gene_data = res.data
        const transcript_list = []
        if (gene_data !== undefined && gene_data.length !== 0 && JSON.stringify(gene_data) !== '{}') {
          const transcript_data = gene_data['transcripts']
          if (transcript_data !== undefined) {
            for (let i = 0; i < transcript_data.length; i++) {
              const item = transcript_data[i]
              if (item['structure'] !== undefined) {
                transcript_list.push(get_trans_data(item['structure'],
                  item['transcript_id'], item['strand']))
              }
            }
          }

          transcript_list.sort().reverse()
          const trans_count = transcript_list.length
          const re_data = get_promoter_and_enhancer_data(gene_data['regulatory_elements'][0]['promoter'],
            gene_data['regulatory_elements'][0]['enhancer'], gene_data['DNMs'])
          const dnms_data = []
          dnms_data.push({
            'ExonicFunc_refGene': gene_data['ExonicFunc_refGene'],
            'Func_refGene': gene_data['Func_refGene'],
            'Gene_refGene': gene_data['Gene_refGene'],
            'alt': gene_data['alt'],
            'chr': gene_data['chr'],
            'end': gene_data['end'],
            'ref': gene_data['ref'],
            'start': gene_data['start'],
            'variant': gene_data['variant_description']
          })
          const dnms_list = get_dnms_data(dnms_data)
          let all_region = get_all_region(transcript_list, re_data)
          all_region = drop_duplicate(all_region)
          all_region = sort_list(all_region)
          const dnm_data = DNM_sort_by_site(dnms_list)
          const all_frag = cut_fragment(all_region)
          const scale_frag = scale_fragment(all_frag, dnm_data, re_data)
          const exton_count = count_exton_region_in_fragment(all_frag, scale_frag, transcript_list)[0]
          const exton_count1 = count_exton_region_in_fragment(all_frag, scale_frag, transcript_list)[1]
          const exton_plot_data = convert_trans_to_xy(exton_count, exton_count1, transcript_list)
          const utr_count = count_utr_region_in_fragment(all_frag, scale_frag, transcript_list)[0]
          const utr_count1 = count_utr_region_in_fragment(all_frag, scale_frag, transcript_list)[1]
          const utr_plot_data = convert_utr_to_xy(utr_count, utr_count1, transcript_list)
          let promoter_plot_data = []
          if (re_data[0].length !== 0) {
            const promoter_count = count_promoter_region_in_fragment(all_frag, scale_frag, re_data)[0]
            // const promoter_count1 = count_promoter_region_in_fragment(all_frag, scale_frag, re_data)[1]
            promoter_plot_data = convert_regulatory_elements_to_xy(promoter_count, 'Promoter', re_data)
          } else {
            promoter_plot_data = undefined
          }
          let enhancer_plot_data = []
          if (re_data[1].length !== 0) {
            const enhancer_count = count_enhancer_region_in_fragment(all_frag, scale_frag, re_data)[0]
            // const enhancer_count1 = count_enhancer_region_in_fragment(all_frag, scale_frag, re_data)[1]
            enhancer_plot_data = convert_regulatory_elements_to_xy(enhancer_count, 'Enhancer', re_data)
          } else {
            enhancer_plot_data = undefined
          }
          const final_dnm = mapping_mutation_to_frag(all_frag, scale_frag, dnm_data)
          const dnm_plot_data = convert_dnms_to_xy(final_dnm)
          const trace = []
          let count = 0
          let max_x = 0
          // const trace2 = []
          for (let i = 0; i < exton_plot_data.length; i++) {
            const item = exton_plot_data[i]
            const trace_item = {
              type: 'scatter',
              visible: true,
              hoverinfo: 'text',
              hoverlabel: {
                'font': {
                  'size': 10
                }
              },
              showlegend: false,
              mode: 'lines',
              x: [item[0][0], item[1][0]],
              y: [item[0][1], item[1][1]],
              text: 'Exon, ' + '[' + String(item[0][2]) + '-' + String(item[1][2]) + ']',
              line: {
                color: 'rgb(72,130,180)',
                shape: 'linear',
                width: 10,
                simplify: true
              }
            }
            trace.push(trace_item)
            count += 1
            if (item[1][0] > max_x) {
              max_x = item[1][0]
            }
          }
          for (let i = 0; i < utr_plot_data.length; i++) {
            const item = utr_plot_data[i]
            const trace_item = {
              type: 'scatter',
              visible: true,
              hoverinfo: 'text',
              hoverlabel: {
                'font': {
                  'size': 10
                }
              },
              showlegend: false,
              mode: 'lines',
              x: [item[0][0], item[1][0]],
              y: [item[0][1], item[1][1]],
              text: 'Untranslated region, ' + '[' + String(item[0][2]) + '-' + String(item[1][2]) + ']',
              line: {
                color: 'gray',
                shape: 'linear',
                width: 5
              }
            }
            trace.push(trace_item)
            count += 1
            if (item[1][0] > max_x) {
              max_x = item[1][0]
            }
          }
          if (promoter_plot_data.length !== 0) {
            for (let i = 0; i < promoter_plot_data.length; i++) {
              const item = promoter_plot_data[i]
              const trace_item = {
                type: 'scatter',
                visible: true,
                hoverinfo: 'text',
                hoverlabel: {
                  'font': {
                    'size': 10
                  }
                },
                showlegend: false,
                mode: 'lines',
                x: [item[0][0], item[1][0]],
                y: [item[0][1], item[1][1]],
                text: String(item[0][2]),
                line: {
                  color: get_promoter_color(String(item[0][2])),
                  shape: 'linear',
                  width: 10,
                  simplify: true
                }
              }
              trace.push(trace_item)
              count += 1
              if (item[1][0] > max_x) {
                max_x = item[1][0]
              }
            }
          }
          if (enhancer_plot_data.length !== 0) {
            for (let i = 0; i < enhancer_plot_data.length; i++) {
              const item = enhancer_plot_data[i]
              const trace_item = {
                type: 'scatter',
                visible: true,
                hoverinfo: 'text',
                hoverlabel: {
                  'font': {
                    'size': 10
                  }
                },
                showlegend: false,
                mode: 'lines',
                x: [item[0][0], item[1][0]],
                y: [item[0][1], item[1][1]],
                text: String(item[0][2]),
                line: {
                  color: get_enhancer_color(String(item[0][2])),
                  shape: 'linear',
                  width: 10,
                  simplify: true
                }
              }
              trace.push(trace_item)
              count += 1
              if (item[1][0] > max_x) {
                max_x = item[1][0]
              }
            }
          }
          for (let i = 0; i < dnm_plot_data.length; i++) {
            const item = dnm_plot_data[i]
            const type = item[0][2].split(',')[2]
            const c = get_DNM_color(type)
            const trace_item = {
              type: 'scatter',
              visible: true,
              hoverinfo: 'text',
              hoverlabel: {
                'font': {
                  'size': 13
                }
              },
              showlegend: false,
              mode: 'lines',
              x: [item[0][0], item[1][0]],
              y: [item[0][1], item[1][1]],
              text: String(item[0][2]),
              line: {
                color: c,
                shape: 'linear',
                width: 10,
                simplify: true
              }
            }
            trace.push(trace_item)
            count += 1
            if (item[1][0] > max_x) {
              max_x = item[1][0]
            }
          }
          const l = []
          for (let i = 0; i < exton_plot_data.length; i++) {
            const item = exton_plot_data[i]
            if ((dnm_plot_data[0][0][0] > item[0][0] && dnm_plot_data[0][0][0] < item[1][0]) ||
              (dnm_plot_data[0][1][0] > item[0][0] && dnm_plot_data[0][1][0] < item[1][0])) {
              const trace_item = {
                type: 'scatter',
                visible: true,
                hoverinfo: 'text',
                hoverlabel: {
                  'font': {
                    'size': 10
                  }
                },
                showlegend: false,
                mode: 'lines',
                x: [item[0][0], item[1][0]],
                y: [item[0][1], item[1][1]],
                text: 'Exon, ' + '[' + String(item[0][2]) + '-' + String(item[1][2]) + ']',
                line: {
                  color: 'rgb(72,130,180)',
                  shape: 'linear',
                  width: 10,
                  simplify: true
                }
              }
              trace.push(trace_item)
              console.log(item)
              l.push('exton')
              l.push(item[0][1])
            }
          }
          for (let i = 0; i < utr_plot_data.length; i++) {
            const item = utr_plot_data[i]
            if ((dnm_plot_data[0][0][0] > item[0][0] && dnm_plot_data[0][0][0] < item[1][0]) ||
              (dnm_plot_data[0][1][0] > item[0][0] && dnm_plot_data[0][1][0] < item[1][0])) {
              const trace_item = {
                type: 'scatter',
                visible: true,
                hoverinfo: 'text',
                hoverlabel: {
                  'font': {
                    'size': 10
                  }
                },
                showlegend: false,
                mode: 'lines',
                x: [item[0][0], item[1][0]],
                y: [item[0][1], item[1][1]],
                text: 'Untranslated region, ' + '[' + String(item[0][2]) + '-' + String(item[1][2]) + ']',
                line: {
                  color: 'gray',
                  shape: 'linear',
                  width: 5
                }
              }
              trace.push(trace_item)
              l.push('utr')
              l.push(item[0][1])
            }
          }
          for (let i = 1; i < l.length; i++) {
            if (l[0] === 'exton') {
              for (let j = 0; j < utr_plot_data.length; j++) {
                const item = utr_plot_data[j]
                if (item[0][1] === l[i]) {
                  const trace_item = {
                    type: 'scatter',
                    visible: true,
                    hoverinfo: 'text',
                    hoverlabel: {
                      'font': {
                        'size': 10
                      }
                    },
                    showlegend: false,
                    mode: 'lines',
                    x: [item[0][0], item[1][0]],
                    y: [item[0][1], item[1][1]],
                    text: 'Untranslated region, ' + '[' + String(item[0][2]) + '-' + String(item[1][2]) + ']',
                    line: {
                      color: 'gray',
                      shape: 'linear',
                      width: 5
                    }
                  }
                  trace.push(trace_item)
                }
              }
            } else if (l[0] === 'utr') {
              for (let j = 0; j < exton_plot_data.length; j++) {
                const item = exton_plot_data[j]
                if (item[0][1] === l[i]) {
                  const trace_item = {
                    type: 'scatter',
                    visible: true,
                    hoverinfo: 'text',
                    hoverlabel: {
                      'font': {
                        'size': 10
                      }
                    },
                    showlegend: false,
                    mode: 'lines',
                    x: [item[0][0], item[1][0]],
                    y: [item[0][1], item[1][1]],
                    text: 'Exon, ' + '[' + String(item[0][2]) + '-' + String(item[1][2]) + ']',
                    line: {
                      color: 'rgb(72,130,180)',
                      shape: 'linear',
                      width: 10,
                      simplify: true
                    }
                  }
                  trace.push(trace_item)
                }
              }
            }
          }
          if (promoter_plot_data.length !== 0) {
            for (let i = 0; i < promoter_plot_data.length; i++) {
              const item = promoter_plot_data[i]
              if ((dnm_plot_data[0][0][0] > item[0][0] && dnm_plot_data[0][0][0] < item[1][0]) ||
                (dnm_plot_data[0][1][0] > item[0][0] && dnm_plot_data[0][1][0] < item[1][0])) {
                const trace_item = {
                  type: 'scatter',
                  visible: true,
                  hoverinfo: 'text',
                  hoverlabel: {
                    'font': {
                      'size': 10
                    }
                  },
                  showlegend: false,
                  mode: 'lines',
                  x: [item[0][0], item[1][0]],
                  y: [item[0][1], item[1][1]],
                  text: 'Exon, ' + '[' + String(item[0][2]) + '-' + String(item[1][2]) + ']',
                  line: {
                    color: 'rgb(72,130,180)',
                    shape: 'linear',
                    width: 10,
                    simplify: true
                  }
                }
                trace.push(trace_item)
              }
            }
          }
          if (enhancer_plot_data.length !== 0) {
            for (let i = 0; i < enhancer_plot_data.length; i++) {
              const item = enhancer_plot_data[i]
              if ((dnm_plot_data[0][0][0] > item[0][0] && dnm_plot_data[0][0][0] < item[1][0]) ||
                (dnm_plot_data[0][1][0] > item[0][0] && dnm_plot_data[0][1][0] < item[1][0])) {
                const trace_item = {
                  type: 'scatter',
                  visible: true,
                  hoverinfo: 'text',
                  hoverlabel: {
                    'font': {
                      'size': 10
                    }
                  },
                  showlegend: false,
                  mode: 'lines',
                  x: [item[0][0], item[1][0]],
                  y: [item[0][1], item[1][1]],
                  text: 'Exon, ' + '[' + String(item[0][2]) + '-' + String(item[1][2]) + ']',
                  line: {
                    color: 'rgb(72,130,180)',
                    shape: 'linear',
                    width: 10,
                    simplify: true
                  }
                }
                trace.push(trace_item)
              }
            }
          }
          for (let i = 0; i < dnm_plot_data.length; i++) {
            const item = dnm_plot_data[i]
            const type = item[0][2].split(',')[2]
            const c = get_DNM_color(type)
            const trace_item = {
              type: 'scatter',
              visible: true,
              hoverinfo: 'text',
              hoverlabel: {
                'font': {
                  'size': 13
                }
              },
              showlegend: false,
              mode: 'lines',
              x: [item[0][0], item[1][0]],
              y: [item[0][1], item[1][1]],
              text: String(item[0][2]),
              line: {
                color: c,
                shape: 'linear',
                width: 10,
                simplify: true
              }
            }
            trace.push(trace_item)
          }
          const visible1 = []
          const visible2 = []
          for (let i = 0; i < count; i++) {
            visible1.push(true)
            visible2.push(false)
          }
          for (let i = 0; i < trace.length - count; i++) {
            visible1.push(false)
            visible2.push(true)
          }
          const updatemenus = [{
            type: 'buttons',
            active: -1,
            pad: {
              r: 100,
              t: 50
            },
            buttons: [{
              label: '<span style="text-align: center">all mRNA isoforms</span>',
              method: 'update',
              args: [{ 'visible': visible1 },
                {
                  'title': 'all mRNA isoforms'
                }]
            },
            {
              label: 'mRNA isoforms with <br>coding regions being impact',
              method: 'update',
              args: [{ 'visible': visible2 },
                {
                  'title': 'mRNA isoforms with coding regions being impact'
                }]
            }]
          }]
          const layout = {
            updatemenus: updatemenus,
            paper_bgcolor: 'rgb(249, 249, 249)',
            plot_bgcolor: 'rgb(249, 249, 249)',
            width: 1200,
            height: get_heights(trans_count),
            hovermode: 'closest',
            spikedistance: 0,
            hoverdistance: 2,
            xaxis: {
              range: [0, max_x],
              showgrid: false,
              zeroline: false,
              showline: false,
              showticklabels: false,
              autorange: false,
              dtick: 100
            },
            yaxis: {
              titlefont: {
                family: 'Arial, sans-serif',
                size: 20,
                color: 'lightgrey'
              },
              showticklabels: true,
              tickangle: 360,
              tickfont: {
                family: 'Arial, serif',
                size: 12,
                color: 'black'
              },
              exponentformat: 'e',
              showexponent: 'ALL'
            },
            margin: {
              l: 130,
              r: 5,
              t: 30,
              pad: 0
            }
          }
          this.data = trace
          this.layout = layout
          this.isPlot = true
          this.loading = false
        } else {
          this.isPlot = false
          this.loading = false
        }
      })
    }
  },
  // mounted() {
  //   this.variant_key = this.$route.query.variant
  //   if (this.variant_key) {
  //     this.isformImpactPlot(this.variant_key)
  //   }
  //   this.isformImpactPlot()
  // },
  // created() {
  //   this.isformImpactPlot()
  // }
}
</script>
